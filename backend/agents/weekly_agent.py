    ...
    async def run(self, user_id, site_url, website_number_of_pages, website_type, user_goal,
        ...
        try:
            ...
            snapshot = await collect_search_console_data(
                access_token=credentials["access_token"], site_url=site_url,
                start_date=start.isoformat(), end_date=end.isoformat(),
            )
            snapshot["query_pages"] = await self.reviewer.search_console.query_pages(
                credentials["access_token"], site_url, start, end,
            )
            ...
            reviews = await self.reviewer.review(site_url, history, pages, credentials["access_token"])
            limitations = ["Raw HTML sample only; no browser rendering, full-site crawl or live Google indexing test.",
                           "User completion time is a proxy for deployment. Performance comparisons show association, not causation.",
                           "Search Console returns a limited set of rows; omitted rows are not proof of zero traffic."]
            ...
            if settings.SEO_DEMO_MODE:
                limitations.append("Demo model budget: only a small evidence sample reaches the model. Full observations remain saved.")
            ...
            if evidence["prompt_omissions"]:
                limitations.append("Model input omitted records: " + json.dumps(evidence["prompt_omissions"]) + ".")
