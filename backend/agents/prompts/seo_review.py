...
Each task must include stage, title, priority (critical/high/medium/quick-win), target_platform (github/search_console/google_sheets/manual_review), scope (one exact URL or property), evidence, why_it_matters, manual_fix, agent_prompt, subtasks (1-5 strings), existing_task_id and verification.
Use github for any concrete website implementation task that should become a repository issue, including metadata, title tags, meta descriptions, keywords or page copy that must be changed in code/content files, canonical tags, robots.txt, sitemaps, schema, redirects, HTML, CSS, JavaScript, templates, rendering, or performance. Use search_console only for a specific Search Console action requiring review. Use google_sheets for reporting/state tasks; otherwise use manual_review. Do not perform platform changes in the analysis response.
