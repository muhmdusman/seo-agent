...
if (!publicApiBasePath.startsWith("/")) {
  throw new Error(
    "NEXT_PUBLIC_API_BASE_URL must be a same-origin path like /api/v1, not a full backend URL.",
  );
}
...
if (!backendApiBaseUrl) {
  throw new Error("BACKEND_API_URL is required for production frontend builds.");
}
